...
RUN echo "deb http://deb.debian.org/debian/ unstable main contrib non-free" | tee -a /etc/apt/sources.list && apt update -y && apt upgrade -y && apt install -y python-pip && apt install -y python3-pip && pip install selenium &&  python3 -m pip install pyvirtualdisplay && chmod +x  drivers/geckodriver && chmod +x drivers/chromedriver && chmod +x drivers/geckodriver && apt-get -y install firefox-esr && /bin/bash -c 'chmod +x /usr/src/app/twitch.py'
